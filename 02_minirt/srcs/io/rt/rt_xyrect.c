...
/*   Updated: 2021/02/12 00:35:49 by sehpark          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_list	*xyrect_node(t_xyrect rectangle, int texture)
{
	t_xyrect	*p_rect;
	t_object	*p_ob;
	t_list		*p_node;
	...
	if (!(p_rect = xyrect(rectangle)))
		return (NULL);
	if (!(p_ob = object((void *)p_rect, OB_XYRECT, texture)))
	{
		free(p_rect);
		return (NULL);
	}
...
void		rt_xyrect(t_minirt *rt)
{
	...
	if (!(p_node = xyrect_node(rectangle, texture)))
		error_handle(-3, rt);
