...
/*   Updated: 2021/02/12 01:10:46 by sehpark          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_list	*xzrect_node(t_xzrect rectangle, int texture)
{
	t_xzrect	*p_rect;
	t_object	*p_ob;
	t_list		*p_node;
	...
	if (!(p_rect = xzrect(rectangle)))
		return (NULL);
	if (!(p_ob = object((void *)p_rect, OB_XZRECT, texture)))
	{
		free(p_rect);
		return (NULL);
	}
...
void		rt_xzrect(t_minirt *rt)
{
	...
	if (!(p_node = xzrect_node(rectangle, texture)))
		error_handle(-3, rt);
